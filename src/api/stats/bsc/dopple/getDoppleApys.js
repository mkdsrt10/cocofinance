...
const getDoppleApys = async () => {
  ...
  let promises = [];
  pools.filter(pool => pool.platform === "Dopple").forEach(pool => promises.push(getPoolApy(masterchef, pool, baseApys)));
...
const getPoolApy = async (masterchef, pool, baseApys) => {
  ...
  if (pool.tokenAddress) {
    getTotalStaked = getTotalStakedInUsd(masterchef, pool.tokenAddress, pool.oracle, pool.oracleId);
  } else {
  ...
  // console.log("APR: "+pool.name, simpleApy)
  const apy = incorporateBifiFee(simpleApy, process.env.BASE_HPY, 1, 0.955);
